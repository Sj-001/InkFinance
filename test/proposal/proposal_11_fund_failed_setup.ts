...
describe("proposal related test", function () {
    ...
    it("test create investment-setup proposal ", async function () {
        ...
        await withdrawPrincipal(fundManagerAddress, erc20Address);
    ...
    async function withdrawPrincipal(fundManagerAddress:string, erc20Address:string) {
        const signers = await ethers.getSigners();
        ...
        await fundManager.triggerFundLaunchStatus(funds[0])

        console.log("fund status:", await fundManager.getFundStatus(funds[0]))
        ...
        console.log("principal claimable:", await fundManager.getCurrentInvestment(funds[0],buyer1.address))
        const erc20 = await ethers.getContractAt("InkERC20", erc20Address);
        

        console.log("before claim balance:", await erc20.balanceOf(buyer1.address))

        await fundManager.withdrawPrincipal(funds[0]);
